...
export default class MovieController {
  ...
  public save = async (req: Request, res: Response): Promise<void> => {
    ...
    if (!movieId || isNaN(parseInt(movieId))) {
      res
        .status(500)
        .json({ message: "El id no es correcto", id: req.params.id });
    ...
    try {
      ...
      const genres = await genreController.save(
      ...
      const newMovie = new Movie({
        ...
        premiered: movieData.release_date
      ...
      res.status(200).json({
        message: `Película ${savedMovie.title} insertada con éxito.`,
        movie: savedMovie,
      });
    } catch (error) {
